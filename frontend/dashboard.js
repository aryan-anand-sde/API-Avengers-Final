...
document.getElementById("add-medicine-form").addEventListener("submit", async (e) => {
  ...
  const formattedTime = new Date(`2000-01-01T${timeRaw}`).toLocaleTimeString("en-US", {
    hour: "numeric",
    minute: "numeric",
    hour12: true,
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadMedicines() {
    try {
      ...
      console.error(err);
      medicinesList.innerHTML = '<div class="error">Could not load medicines. Make sure backend is running.</div>';
    }
  }
  ...
  const renderMedicines = () => {
    ...
    if (taken) statusHtml = `<span class="status-pill status-on-time">Taken</span>`;
    else if (medicine.taken === false) statusHtml = `<span class="status-pill status-late">Missed</span>`;
    ...
    const saveDoseTicks = (medId, arr) => {
      try { localStorage.setItem(getTodayKey(medId), JSON.stringify(arr)); } catch (e) { console.warn('Could not save dose ticks', e); }
    };

    const doseTicks = loadDoseTicks(medicine._id, timesArray.length);

    // build boxes HTML
    const boxesHtml = timesArray.map((t, idx) => {
      const checked = doseTicks[idx] ? 'checked' : '';
      const label = t || `Dose ${idx + 1}`;
      return `<label class="dose-box" data-med="${medicine._id}" data-idx="${idx}"><input type="checkbox" class="dose-checkbox" ${checked}><span class="dose-label">${escapeHtml(label)}</span></label>`;
    }).join('');
    ...
    medicineItem.innerHTML = `
        <div class="medicine-content">
          ...
          <p class="time-line">Time: ${escapeHtml(medicine.time || (timesArray[0] || '—'))}</p>
          ${takenAt ? `<p class="taken-at">Taken at: ${takenAt.toLocaleString()}</p>` : ''}
          <div class="dose-boxes">${boxesHtml}</div>
        </div>
        <div class="medicine-actions">
          <button class="btn" data-id="${medicine._id}" data-action="taken">${taken ? 'Taken' : 'Mark Taken'}</button>
          <button class="btn" data-id="${medicine._id}" data-action="missed">${!taken ? 'Missed' : 'Mark Missed'}</button>
          <button class="btn btn-delete" data-id="${medicine._id}" data-action="delete"><i class="fas fa-trash"></i></button>
        </div>
      `;
  ...
  async function loadAnalytics(startDate, endDate) {
    try {
      ...
      if (container) container.innerHTML = '<div class="error">Could not load analytics</div>';
    }
  }
  ...
  function renderDailySummary(daily) {
    ...
    if (!daily || daily.length === 0) {
      container.innerHTML = '<div class="empty">No daily data for selected range</div>';
      return;
    }
